...
fn stage_newer_release() -> Result<(), String> {
    ...
    let response: serde_json::Value = ureq::get(RELEASE_URL)
    ...
    let url = assets
        .iter()
        .find(|a| a.get("name").and_then(serde_json::Value::as_str) == Some(PACKAGE))
    ...
    let checksum_url = assets
        ...
        .find(|a| {
            a.get("name").and_then(serde_json::Value::as_str)
                == Some("CommandBlock-Windows-x64.zip.sha256")
        })
        .and_then(|a| a.get("browser_download_url"))
        .and_then(serde_json::Value::as_str)
        .ok_or("ไม่พบ checksum แพ็กเกจ")?;
    let bytes = read_bytes(url)?;
    let expected = String::from_utf8(read_bytes(checksum_url)?)
        .map_err(|e| e.to_string())?
        .split_whitespace()
        .next()
        .unwrap_or("")
