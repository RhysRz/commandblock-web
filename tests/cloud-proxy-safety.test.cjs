...
test('Pages deploy publishes only the web directory', () => {
  const source = fs.readFileSync(path.join(__dirname, '..', '.github', 'workflows', 'deploy-pages.yml'), 'utf8'); assert.match(source, /path: web/);
});
